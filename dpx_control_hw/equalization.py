...
class Equalization(dpx_functions.DPXFunctions):
    ...
    def threshold_equalization(self,
        ...
        if i_pixeldac is not None:
            periphery_d = support.split_perihpery_dacs(self.dpx.periphery_dacs)
            periphery_d['i_pixeldac'] = i_pixeldac
            self.dpx.periphery_dacs = support.perihery_dacs_dict_to_code(periphery_d)

        # Get pixel dac settings
        if num_dacs <= 2:
            num_dacs = 2
        pixel_dacs = np.asarray(np.linspace(0, 63, num_dacs), dtype=int)
        pixel_dac_settings = ['%02x' % pixel_dac for pixel_dac in pixel_dacs]
        ...
        # New THL
        thl_mean = int(sum([mean_dict[pixel_dac] for pixel_dac in pixel_dac_settings]) // num_dacs)
        ...
        noise_thl_first = np.asarray(noise_thl_first)
        noise_thl_second = np.asarray(noise_thl_second)
        ...
        if not use_gui and plot:
            min_range = min([min(noise_thl[pixel_dac]) for pixel_dac in pixel_dac_settings])
            max_range = max([max(noise_thl[pixel_dac]) for pixel_dac in pixel_dac_settings])
